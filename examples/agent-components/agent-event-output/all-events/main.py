...
element. Tool-call fragments are rendered as individual atomic events; the
finalized call remains a separate event containing the complete arguments.
"""
...
def _title(origin, category):
    name = str(origin.get("subagent_name") or origin.get("main_agent_name") or "")
    if not name:
        raise ValueError("Agent Event Output origin is missing its Agent name")
    return f"Agent [{name}] · {category}"
