"""Render every currently exposed Workflow event as readable HTML.

Only assistant text and reasoning use streamed markup. All Tool, lifecycle,
State, custom, media, Workflow Run, and future events return one complete
``details`` element. ``segment_end`` is optional presentation markup: the
scheduler stores its returned string but never assumes that it is HTML or that
the selected opening text has a particular closing tag.
"""
...
def render_assistant_text_segment_end(event, origin, payload, metadata):
    """Close an assistant-text presentation segment released by the scheduler."""
...
def render_workflow_run_start(event, origin):
    """The Shell-owned Workflow Run started."""

    return _atomic("Run started", event, event, origin, "workflow-run-start")


def render_workflow_run_end(event, origin):
    """The Shell-owned Workflow Run completed."""

    return _atomic("Run finished", event, event, origin, "workflow-run-end")


def render_workflow_run_error(event, origin):
    """The Shell-owned Workflow Run failed."""

    return _atomic("Run error", event, event, origin, "workflow-run-error")
...
def run_output(event, origin):
    """Render the Shell-owned Workflow Run event as one complete block."""

    if event.get("type") != "agent_shell.workflow_run":
        return ""
    phase = str(event.get("phase") or "")
    if phase == "start":
        return render_workflow_run_start(event, origin)
    if phase == "error":
        return render_workflow_run_error(event, origin)
    return render_workflow_run_end(event, origin)
