...
class SystemLogSettingsStore:
    ...
    def _validated_value(value: object) -> int:
        if isinstance(value, bool) or not isinstance(value, int):
            raise ValueError("system log maximum size must be an integer")
        if value < MIN_SYSTEM_LOG_MAX_SIZE_MIB:
    ...
    def snapshot(self) -> dict[str, int]:
        value = self._validated_value(
            ...
            .get("system_log", {})
            .get("max_size_mib", DEFAULT_SYSTEM_LOG_MAX_SIZE_MIB)
        )
    ...
    def set_max_size_mib(self, max_size_mib: int) -> dict[str, int]:
        max_size_mib = self._validated_value(max_size_mib)
        self._repository.update_system(
            lambda system: system.setdefault("system_log", {}).__setitem__(
