...
class McpToolStore:
    ...
    def list_items(self, *, enabled_only: bool = False) -> list[dict]:
        records = [
            ...
            for item in self._repository.list_records(
                "mcp_tools", fields=self._PUBLIC_FIELDS
            )
        ]
        if enabled_only:
    ...
    def list_item_summaries(self) -> list[dict]:
        return self.list_items()

    def get_item(self, item_id: str) -> dict | None:
        item = self._repository.get_record(
            "mcp_tools", item_id, fields=self._PUBLIC_FIELDS
        )
    ...
    def get_item_by_name(self, name: str) -> dict | None:
        item = self._repository.find_record(
            "mcp_tools", "name", name, fields=self._PUBLIC_FIELDS
        )
        return self._public(item) if item is not None else None
    ...
    def save_item(
        ...
        *,
        ...
        def mutate(config: dict) -> None:
            records = config.setdefault("mcp_tools", [])
            if any(
                item.get("name") == data["name"] and item.get("id") != item_id
                for item in records
            ):
                raise ValueError("MCP Tool name already exists")
            ...
            for index, item in enumerate(records):
                if item.get("id") == item_id:
                    stored["definition"] = deepcopy(
                        item.get("definition", empty_definition)
                    )
    ...
    def get_graph(self, item_id: str) -> WorkflowGraphDocumentV1 | None:
        item = self._repository.get_record(
            "mcp_tools", item_id, fields=("definition", "layout")
        )
        if item is None:
            return None
    ...
    def save_graph_and_enabled(
        ...
        *,
        ...
        def mutate(config: dict) -> None:
            nonlocal changed
            for item in config.setdefault("mcp_tools", []):
                if item.get("id") == item_id:
