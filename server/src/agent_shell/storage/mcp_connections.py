...
class McpResourceStore:
    ...
    def install_connection(self, connection_id: str) -> dict[str, Any]:
        ...
        mutations only. Installation resolves its input snapshot under the
        per-Connection operation lock, then publishes artifacts under the
        installation owner's own roots, so a multi-minute package download
        does not block unrelated Connection reads or writes.
        """
        ...
        def declaration() -> dict[str, Any]:
            with self._lock:
                return self._snapshot_unlocked().copy_input(connection_id)

        return self._installations.install(connection_id, declaration)
