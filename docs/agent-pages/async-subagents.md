...
Async Subagent 引用不会自行开启异步能力。引用方 Main Agent 还必须显式选择【Async Subagent Middleware / 异步子代理中间件】组件；选择组件但没有引用时保存失败。该组件可设置 Middleware system prompt，并可分别覆盖五个官方工具的 description；空 override 使用当前锁定 Deep Agents 版本的官方默认文本。
