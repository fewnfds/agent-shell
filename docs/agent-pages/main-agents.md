...
Main Agent是由 LangChain `create_agent()` 和显式 middleware assembly 构造的完整、可复用 Agent Server root graph。它可以通过`is_model_entry=true`直接发布为OpenAI-compatible model，也可以由Workflow Command通过`runtime.context.agent_runs`启动独立Thread/Run。
