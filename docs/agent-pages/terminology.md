...
| Agent 角色 | `Main Agent`、`synchronous Subagent` | Main Agent 是 root graph；synchronous Subagent 是其内部委派配置 |
...
| External Agent | 保存由外部 CLI 承载的 agent 预设的配置实体；不进入 LangChain 装配，不产生官方 Thread 或 Run |
