...
【系统 / 系统配置】的 Workflow Debug 采集默认关闭。开启后，新 Run 的结构事件保存经过 JSON 转换且 credential 字段脱敏的完整 LangChain callback metadata；每次 Chain、Model 和 Tool callback failure 都生成运行诊断及完整 exception chain/traceback 附件，因此自动重试成功前的中间异常也可从日志中心和运行 ZIP 检查。Run 同时在 LangGraph v3 post-transformer、pre-normalizer 边界保存应用实际消费的完整 `ProtocolEvent` envelope，包括 `seq`、`method`、`namespace`、timestamp 和 channel data。该记录不补开 transformer 未请求的 Pregel stream mode，也不恢复已被 transformer 抑制的内部事件。该模式增加运行历史、诊断索引和附件体积，关闭后使用轻量采集；已启动 Run 不受中途切换影响。
