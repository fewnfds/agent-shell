...
- 创建时间与 Lifecycle 聚合状态；Lifecycle record 在入口 Run 建立前保存唯一的创建时间、request identity 和入口 Graph identity。Management API 返回带 UTC offset 的 ISO timestamp，管理台按浏览器本地时区显示；
- active/total Run 数量；
...
任意 Lifecycle 都可以进入监控页。active Lifecycle 不能删除，但可以在目录里取消：取消会同时取消该 Lifecycle 的全部 active Run，并结束该 Lifecycle 仍在等待的请求入口响应，发出 OpenAI-compatible 请求的客户端因此收到 `completion_cancelled` 并结束等待。取消只作用于这一个 Lifecycle，不会影响其他并发请求。官方 Thread/Run 状态暂时不可读取时，取消返回 `workflow_lifecycle_status_unavailable`，避免把未执行的取消报告成成功；恢复后可以重试。terminal Lifecycle 可以单项删除或按当前搜索条件批量删除。
