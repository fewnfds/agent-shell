...
【系统 / 系统配置】的 Workflow Debug 采集默认关闭。开启后，新 Run 的结构事件保存经过 JSON 转换且 credential 字段脱敏的完整 LangChain callback metadata，不再只保留常用字段；每次 Chain、Model 和 Tool callback failure 都生成运行诊断及完整 exception chain/traceback 附件，因此自动重试成功前的中间异常也可从日志中心和运行 ZIP 检查。该模式增加运行历史、诊断索引和附件体积，关闭后恢复轻量采集；已启动 Run 不受中途切换影响。
