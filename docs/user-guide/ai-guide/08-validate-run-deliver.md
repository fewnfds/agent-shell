...
Workflow 不装配模型。只要本次交付包含直接入口 Main Agent、Command-launched Agent 或 synchronous Subagent，就检查对应 Model Mapping。
...
检查每个直接运行或被引用的 Main Agent，以及其 synchronous Subagent 使用的 Model Requirement。
