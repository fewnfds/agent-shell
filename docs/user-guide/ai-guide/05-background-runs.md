...
`background_task_id` 和 `run_depth`。这些 identity 从官方 `Runtime.context` 读取；State、Store 和 checkpoint 保存业务所需的序列化字段。
...
background Run 由应用级 Manager 管理。background Agent 使用实际 `agent_id` 且 `workflow_node_id` 为空；background Workflow 由自己的 Workflow 配置标识。Deep Agents synchronous Subagent 和 Task Dispatcher 的 request-scoped dynamic worker 使用各自的 execution semantics，
...
Lifecycle 保留到显式删除；parent 和所有 background task 进入终态后，Lifecycle 接受 explicit delete。Lifecycle 进入 `deleting` status 后冻结 background Run 创建，
