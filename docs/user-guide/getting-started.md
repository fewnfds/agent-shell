...
- 【系统】：组件配置、模型连接、系统配置、拦截消息、日志中心和运行历史；
...
- 【配置库】：通过通用列表查看、复制、编辑、下载、上传和删除配置；系统组列出不提供下载的模型连接；
- 【词库】与【样式实验室】：术语查询和 UI 样式实验。

## 第一份可运行 Workflow

首次启动会创建并激活 `Default` Configuration Repository。需要使用另一套配置时，在【系统 / 组件配置】切换或复制 Repository；之后创建的 Component、Agent 和 Workflow 都写入当前 Repository。
