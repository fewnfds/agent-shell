...
- 【工作流组件】：Workflow Event Output、Command Node 和 Task Dispatcher 配置；
...
3. 在【模型 / 模型映射】为 Model Requirement 选择 Model Connection；未绑定时页面显示 warning，必须完成 binding 后才能运行。然后在【工作流 / 父图】（parent Workflow）新建记录，点击【编辑】进入 Workflow canvas。
