...
- Subagent 的 `name`、模型可见 `description`、`settings.capability_overrides`、`tool_refs` 和 `middleware_refs` 在 Subagent 实体页面维护；Main Agent 另通过 `subagents[].subagent_id` 引用实体。
