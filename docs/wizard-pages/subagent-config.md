...
Main Agent 选择该组件且拥有至少一条有效 Subagent 实体引用时获得 `task`。每个实体由 Shell 投影为 Deep Agents 官方 dictionary-based SubAgent 并同步执行。委派能力只属于 Main Agent；Subagent 不能覆写该组件，也不能定义下级实体引用。
