...
Main Agent 未选择该 capability，或 Subagent 显式保存 `mode: "disabled"` 时，最终 stack 中不包含 Prompt Caching middleware。Subagent 未保存 override 时继承 Main Agent，保存 `replace` 时使用另一份配置。
