...
本目录是配置编辑页的字段索引：前 13 项是 Agent-side Component catalog，其中 Skill Component 只制作独立包；后 5 项是 Workflow-owned 组件。创建与装配见[能力说明](../user-guide/capabilities.md)和[Workflow 配置](../user-guide/configuration-workflow.md)，总体 identity 与引用边界见[Agent Shell 系统契约](../../.docs/architecture/agent-shell-system-contract.md)。
...
前 13 行是 Agent-side Component 的固定 catalog order，末 5 行是 Workflow 组件，不参与该 order。`agent_selectable=false` 的 Skill Component 不出现在 Main Agent 或 Subagent 的 capability picker。
