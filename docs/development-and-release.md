...
文件化 Python 配置扩展中的 `requirements.txt` 不写入 `server/pyproject.toml`，而由 Windows 启动器按可达配置指纹生成 `runtime/python_packages/site-packages`。启用 Workflow 可达集包含 Command、Main Agent/Subagent 引用的 Custom Tool、Custom Middleware、Agent Event Output 和 Workflow Event Output 扩展；静态模板和未触达的配置扩展不参与，输入未变化时复用。扩展层可以增加与核心锁兼容的公开 PyPI 发行包，uv 优先使用兼容 wheel，没有 wheel 时按包的标准构建声明尝试源码构建；扩展依赖不能修改 `runtime/app`。启动设置初始化与读取合并为一次 preflight；扩展依赖准备在最终服务进程内、应用创建前完成，避免为了相邻启动步骤重复拉起并导入 Python 应用。
...
`server/uv.lock` 锁定 Python 发行包。当前稳定基线为：
...
| LangChain adapters | DeepSeek `1.1.1`；Google GenAI `4.4.0`；OpenAI `1.6.6`（Deep Agents 另传递 Anthropic `1.7.4`） |
| Google GenAI Python SDK | `google-genai 2.25.0`（由 LangChain Google GenAI adapter 引入） |
| MCP | `langchain-mcp-adapters 0.3.2`；Python SDK `mcp 1.30.0` |
| LangChain core/graph | `langchain 1.4.2`；`langchain-core 1.6.5`；`langgraph 1.2.12`；LangSmith `0.14.0` |
| LangGraph Dev | CLI `0.4.32`；API `0.14.4`；in-memory runtime `0.34.1`；SDK `0.4.5` |
| 前端 LangChain | `@langchain/core 1.2.12`；`@langchain/vue 1.1.2`；LangGraph SDK `1.11.2` |
...
无范围的 `uv lock --upgrade` 会把 FastAPI、Provider SDK 和开发依赖等无关行为面一起带入，不作为日常升级入口。依赖升级先用 `uv tree --outdated` 扫描，再按单一影响面使用 scoped `--upgrade-package` 推进。LangChain 系的版本边界和下一次复核步骤见[LangChain 系依赖升级](langchain-dependency-upgrades.md)；升级 Deep Agents 还须逐项复核 [Deep Agents 公共组件接入清单](deep-agents-customizations.md)。
