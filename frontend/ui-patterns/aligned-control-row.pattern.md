...
- 真实表单中 switch 与 label-input 并排时使用 `row g-3`、列 class 和 `data-ui-control-row`；每列提供 `.form-label`、`FormField` 或带 label 的已批准控件，switch 列标题也遵循同一可见标签语义。
...
机器门禁见 `ui-policy.json` 的 `styles.controlSemantics` 与 `layoutRules.forbidAdjacentRows`。

Reference: `src/pages/StyleBaselinePage.vue`、`src/pages/EventFeedPage.vue`、`src/pages/WorkflowsPage.vue`。路径相对于 `frontend/`。
