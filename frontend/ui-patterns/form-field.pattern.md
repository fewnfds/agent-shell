...
Use: 一个控件需要按 payload path 解析的统一 label，以及可选 help/error。Source of truth: `src/components/FormField.vue`；policy: `localComponents.approved[name=FormField]`。单位输入见 `input-with-unit.pattern.md`，并排字段见 `aligned-control-row.pattern.md`。
...
Reference / Verify: `src/pages/SystemSettingsPage.vue`、`src/editors/FilesystemEditor.vue`、`src/components/RecordPicker.vue`；完整调用清单见 `ui-policy.json` 的 `localComponents.approved[name=FormField].callers`。路径相对于 `frontend/`。
