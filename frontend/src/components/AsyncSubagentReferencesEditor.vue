...
function profileFor(id: string): AsyncSubagentProfile | undefined {
  return props.profiles.find((profile) => profile.id === id)
}
</script>
  ...
  <section class="card mb-3" aria-labelledby="async-subagent-references-title">
    ...
    <div v-else class="list-group list-group-flush">
      ...
      >
        ...
        <label class="form-label" :for="`async-subagent-profile-${index}`">
          {{ t('agents.asyncSubagent.reference') }}
        </label>
        <select
          :id="`async-subagent-profile-${index}`"
          class="form-select"
          data-testid="async-subagent-profile"
          :value="reference.async_subagent_id"
        ...
        >
          ...
          <option
            v-if="reference.async_subagent_id && !profileFor(reference.async_subagent_id)"
            disabled
            :value="reference.async_subagent_id"
          >
            {{ t('common.missingConfiguration', { id: reference.async_subagent_id }) }}
          </option>
          <option v-for="profile in profiles" :key="profile.id" :value="profile.id">
            {{ profile.component_name }}
          </option>
        </select>
        <p v-if="profileFor(reference.async_subagent_id)" class="small text-body-secondary mb-0 mt-2">
          <strong>{{ profileFor(reference.async_subagent_id)?.name }}</strong>
          <span>{{ t('common.detailSeparator') }}{{ profileFor(reference.async_subagent_id)?.description }}</span>
        </p>
      </div>
    </div>
  </section>
