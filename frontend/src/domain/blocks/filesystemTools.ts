...
export const filesystemToolsAdapter = {
  blank(defaults: FilesystemToolsDefaults): FilesystemToolsDraft {
    return { id: '', name: '', tool_token_limit_before_evict: defaults.tool_token_limit_before_evict, human_message_token_limit_before_evict: defaults.human_message_token_limit_before_evict ?? 50_000, grep_max_count: defaults.grep_max_count ?? 1_000, max_execute_timeout: defaults.max_execute_timeout ?? 120, tool_configs: configs(undefined, defaults) }
  },
  fromApi(value: FilesystemToolsApiRecord, defaults: FilesystemToolsDefaults): FilesystemToolsDraft {
    return { ...identity(value), tool_token_limit_before_evict: normalizeLimit(value.tool_token_limit_before_evict, defaults.tool_token_limit_before_evict), human_message_token_limit_before_evict: normalizeLimit(value.human_message_token_limit_before_evict, defaults.human_message_token_limit_before_evict ?? 50_000), grep_max_count: normalizeRequiredLimit(value.grep_max_count, defaults.grep_max_count ?? 1_000), max_execute_timeout: normalizeRequiredLimit(value.max_execute_timeout, defaults.max_execute_timeout ?? 120), tool_configs: configs(value.tool_configs, defaults) }
  },
  toPayload(value: FilesystemToolsDraft, defaults: FilesystemToolsDefaults): BlockPayloadBase & Record<string, unknown> {
    ...
    return {
      ...
      max_execute_timeout: normalizeRequiredLimit(value.max_execute_timeout, defaults.max_execute_timeout ?? 120),
