    ...
    <section class="card mb-3">
      ...
      <div class="card-body">
          ...
          <div class="col-md-6">
            <FormField field-path="filesystem_import" label-key="editors.filesystemPermissions.importFilesystem">
              <div class="input-group">
                ...
                <LteButton class="action-button" :disabled="!importFilesystemId" data-action="import-filesystem-paths" type="button" @click="importFilesystemPaths">
        ...
        <div v-if="draft.permissions.length" class="simple-mapping-list mt-3">
          <div v-for="(entry, index) in draft.permissions" :key="index" class="simple-mapping-row" data-testid="filesystem-permission-row">
            ...
            <div class="simple-mapping-actions">
              <LteButton class="icon-action-button" :aria-label="t('editors.common.moveUp')" :disabled="index === 0" :title="t('editors.common.moveUp')" size="sm" type="button" @click="movePermission(index, -1)"><i class="bi bi-arrow-up" aria-hidden="true" /></LteButton>
              <LteButton class="icon-action-button" :aria-label="t('editors.common.moveDown')" :disabled="index === draft.permissions.length - 1" :title="t('editors.common.moveDown')" size="sm" type="button" @click="movePermission(index, 1)"><i class="bi bi-arrow-down" aria-hidden="true" /></LteButton>
              <LteButton class="icon-action-button" :aria-label="t('editors.common.remove')" :title="t('editors.common.remove')" data-action="remove-filesystem-permission" size="sm" type="button" @click="draft.permissions.splice(index, 1)"><i class="bi bi-trash" aria-hidden="true" /></LteButton>
            </div>
          </div>
        </div>
        ...
        <div class="simple-mapping-footer">
          <LteButton class="icon-action-button" :aria-label="t('editors.common.add')" :title="t('editors.common.add')" data-action="add-filesystem-permission" size="sm" type="button" @click="addPermission">
