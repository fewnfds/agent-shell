  ...
  <div data-editor="skill">
    <section class="card mb-3">
      ...
      <div class="card-body">
        ...
        <div class="d-flex justify-content-end mb-3"><LteButton class="action-button" :disabled="!draft.system_prompt_enabled" theme="warning" @click="draft.instruction_override = defaults.system_prompt"><i class="bi bi-arrow-clockwise" aria-hidden="true" />{{ t('editors.common.restoreDefault') }}</LteButton></div>
    ...
    <div class="row g-3 align-items-start">
      ...
      <section class="col-lg-6">
        <div class="card h-100">
          <header class="card-header d-flex align-items-center gap-2"><h3 class="card-title">{{ t('editors.skill.privateTitle') }}</h3><LteButton class="icon-action-button ms-auto" :aria-label="t('editors.common.refresh')" :disabled="privateLoading || !draft.id" :title="t('editors.common.refresh')" theme="secondary" type="button" @click="emit('refresh')"><span v-if="privateLoading" class="spinner-border spinner-border-sm" aria-hidden="true" /><i v-else class="bi bi-arrow-clockwise" aria-hidden="true" /><span class="visually-hidden">{{ t('editors.common.refresh') }}</span></LteButton></header>
          <div v-if="(privatePackage?.catalog.length ?? pendingTemplates.length)" class="list-group list-group-flush"><article v-for="skill in (privatePackage?.catalog ?? pendingTemplates)" :key="skill.folder" class="list-group-item" data-testid="private-skill-item"><div class="d-flex align-items-start gap-2"><div class="w-100 text-break"><strong class="d-block">{{ skill.name }}</strong><details v-if="skill.description" class="mt-2"><summary>{{ t('editors.skill.description') }}</summary><p class="mb-0 mt-2 text-body-secondary">{{ skill.description }}</p></details></div><LteButton class="icon-action-button" :aria-label="t('editors.skill.remove')" :disabled="mutating" :title="t('editors.skill.remove')" theme="danger" type="button" @click="removeSkill(removalKey(skill))"><i class="bi bi-trash" aria-hidden="true" /></LteButton></div></article></div>
