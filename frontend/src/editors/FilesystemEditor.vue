        ...
        <div v-if="draft.mapped_directories.length" class="filesystem-mapping-list">
          <div v-for="(item, index) in draft.mapped_directories" :key="index" class="filesystem-mapping-row" data-testid="mapped-directory-row">
            ...
            <div class="filesystem-mapping-actions">
              <LteButton class="icon-action-button" :aria-label="t('editors.common.remove')" :title="t('editors.common.remove')" size="sm" type="button" @click="draft.mapped_directories.splice(index, 1)"><i class="bi bi-trash" aria-hidden="true" /></LteButton>
            </div>
          </div>
        </div>
        <p v-else class="text-body-secondary mb-0">{{ t('editors.filesystem.emptyMappedDirectories') }}</p>
        <div class="simple-mapping-footer">
              <LteButton class="icon-action-button" :aria-label="t('editors.common.add')" :title="t('editors.common.add')" data-action="add-mapped-directory" size="sm" type="button" @click="draft.mapped_directories.push({ virtual_path: '', local_path: '', path_origin: 'absolute', lifecycle_mode: 'fixed' })"><i class="bi bi-plus-lg" aria-hidden="true" /></LteButton>
        </div>
      </div>
    </section>
      ...
      <div class="card-body">
        <div v-if="draft.virtual_directories.length" class="filesystem-mapping-list">
          <div v-for="(item, index) in draft.virtual_directories" :key="index" class="filesystem-mapping-row" data-testid="virtual-directory-row">
            ...
            <div class="filesystem-mapping-actions">
              <LteButton class="icon-action-button" :aria-label="t('editors.common.remove')" :title="t('editors.common.remove')" size="sm" type="button" @click="draft.virtual_directories.splice(index, 1)"><i class="bi bi-trash" aria-hidden="true" /></LteButton>
            </div>
          </div>
        </div>
        <p v-else class="text-body-secondary mb-0">{{ t('editors.filesystem.emptyVirtualDirectories') }}</p>
        <div class="simple-mapping-footer">
              <LteButton class="icon-action-button" :aria-label="t('editors.common.add')" :title="t('editors.common.add')" data-action="add-virtual-directory" size="sm" type="button" @click="draft.virtual_directories.push({ virtual_path: '', source_path: '' })"><i class="bi bi-plus-lg" aria-hidden="true" /></LteButton>
        </div>
      </div>
    </section>
      ...
      <div class="card-body">
        <div v-if="draft.virtual_files.length" class="filesystem-mapping-list">
          <div v-for="(item, index) in draft.virtual_files" :key="index" class="filesystem-mapping-row" data-testid="virtual-file-row">
            ...
            <div class="filesystem-mapping-actions">
              <LteButton class="icon-action-button" :aria-label="t('editors.common.remove')" :title="t('editors.common.remove')" size="sm" type="button" @click="draft.virtual_files.splice(index, 1)"><i class="bi bi-trash" aria-hidden="true" /></LteButton>
            </div>
          </div>
        </div>
        <p v-else class="text-body-secondary mb-0">{{ t('editors.filesystem.emptyVirtualFiles') }}</p>
        <div class="simple-mapping-footer">
              <LteButton class="icon-action-button" :aria-label="t('editors.common.add')" :title="t('editors.common.add')" data-action="add-virtual-file" size="sm" type="button" @click="draft.virtual_files.push({ virtual_path: '', source_path: '' })"><i class="bi bi-plus-lg" aria-hidden="true" /></LteButton>
        </div>
      </div>
    </section>
    ...
    <section class="card mb-3">
      ...
      <div class="card-body">
        <div class="d-flex justify-content-end mb-3">
              <LteButton class="action-button" data-action="restore-default" @click="draft.system_prompt_override = defaults.system_prompt"><i class="bi bi-arrow-clockwise" aria-hidden="true" />{{ t('editors.common.restoreDefault') }}</LteButton>
        </div>
        <LteTextarea
    ...
    <div class="row g-3">
      <div v-for="row in toolRows" :key="row.tool.name" class="col-md-6">
        <article class="card h-100" data-testid="filesystem-tool-card">
          ...
          <div class="card-body">
            <div class="d-flex justify-content-end mb-3">
              <LteButton class="action-button" data-action="restore-default" @click="row.config.description_override = row.tool.default_description"><i class="bi bi-arrow-clockwise" aria-hidden="true" />{{ t('editors.common.restoreDefault') }}</LteButton>
            </div>
            <LteTextarea
