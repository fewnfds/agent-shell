...
export const en: MessageSchema = {
  ...
  workflows: {
    ...
    secondsOrDisabled: 'seconds; blank disables',
    responseStream: {
      title: 'Response Stream',
      action: 'Response Stream',
      back: 'Back to Parent Run Workflows',
      description: 'Configure how events from this Parent Run are serialized into its one append-only OpenAI assistant response.',
      parentOnly: 'Response Stream policy belongs to a Parent Run Workflow.',
      loadFailed: 'Could not load Response Stream policy',
      saveFailed: 'Could not save Response Stream policy',
      saved: 'Response Stream policy saved.',
      errorTitle: 'Response Stream unavailable',
      deliveryTitle: 'Event delivery',
      wrapperTitle: 'Live wrappers',
      wrapperHint: 'Start and end are literal text. There are no templates, placeholders, expressions, or per-delta scripts. A continuation uses the same pair.',
      wrapperStart: 'Literal start',
      wrapperEnd: 'Literal end',
      activityTitle: 'Visible activity',
      sourcesTitle: 'Node visibility overrides',
      noSources: 'This Workflow graph has no Agent, Command, or Task Dispatcher nodes.',
      queue: {
        mode: 'Queue strategy',
        fair: 'Fair Agent turns',
        strict: 'Strict node source',
        grace: 'Successor grace',
        fairHint: 'A live segment remains readable, then ready lanes rotate. A slow Tool releases the writer and its call/result pair returns when complete.',
        strictHint: 'One node invocation keeps its reservation until terminal. Other ready sources may remain queued for minutes.',
      },
      events: {
        assistant_text: 'Assistant text',
        reasoning: 'Reasoning',
        subagent_content: 'Synchronous Subagent content',
        tools: 'Tool call and outcome',
        subagent_lifecycle: 'Synchronous Subagent lifecycle',
        workflow_custom: 'Workflow custom events',
      ...
      delivery: {
        ...
        hidden: 'Hidden',
        paired: 'Atomic call + outcome',
      },
      activity: {
        announceStart: 'Show real start events',
        announceQueued: 'Show when visible work is queued',
        hidden_delta_pulse_seconds: 'Coalesce hidden real deltas',
        quiet_notice_after_seconds: 'First waiting notice after',
        quiet_notice_repeat_seconds: 'Repeat waiting notice every',
      },
      sources: {
        inherit: 'Use event delivery settings',
      ...
      preview: {
        ...
        fairTimeline: 'Agent A → Agent B while Agent A waits for a Tool → completed Tool pair',
        strictTimeline: 'Agent A holds the writer → Agent B stays queued until Agent A is terminal',
        reasoning: 'Agent A reasoning',
        answer: 'Agent A answer',
        toolWait: '[Activity] Agent A: waiting for Tool; no new upstream event is claimed',
        continuation: '[Continued: Agent A] late reasoning delta',
      },
      warnings: {
        liveTitle: 'Live output is irreversible',
        live: 'Live delta bypasses the complete Python event renderer. Already-sent text cannot be filtered, rewritten, removed, or HTML-escaped later.',
        activityTitle: 'Activity is response text',
        activity: 'Visible start, queued, producing, and waiting notices become part of both streaming and non-streaming final assistant content.',
      },
    },
    editor: {
  ...
  errors: {
    ...
    workflowResponseStreamPolicyParentOnly: 'Only a Parent Run Workflow can own a response stream policy.',
    workflowResponseStreamPolicyInvalid: 'The response stream policy is invalid.',
    workflowResponseStreamSourceNotFound: 'A response stream source override refers to a Node that does not exist.',
  ...
  validation: {
    ...
    issue: {
      ...
      workflow: {
        ...
        responseStreamSourceNotFound: 'The response stream source override refers to a Node that does not exist.',
      },
      contract: {
